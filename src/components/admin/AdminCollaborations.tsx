...
const AdminCollaborations = ({ data, profiles, onRefresh }: Props) => {
    ...
    <div className="space-y-3">
      {data.length === 0 ? (
        ...
        <div key={c.id} className="bg-card border border-border rounded-xl p-4 space-y-2">
          ...
          <p className="font-body text-xs text-muted-foreground italic">"{c.reason}"</p>
          {c.what_they_bring && <p className="font-body text-[11px] text-muted-foreground">Brings: {c.what_they_bring}</p>}
          {c.market_relevance && <p className="font-body text-[11px] text-muted-foreground">Market fit: {c.market_relevance}</p>}
          <div className="flex items-center gap-2 pt-1">
            ...
            {(c.status === "submitted" || c.status === "under review") && (
              <>
                <Button size="sm" onClick={() => handleAction(c.id, "approved")} className="bg-emerald-600 hover:bg-emerald-700 text-white font-body text-xs h-7 ml-auto">
